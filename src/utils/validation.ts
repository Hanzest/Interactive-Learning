...
function validateSection(section: any, path: string): { valid: boolean; error?: string } {
  ...
  if (type === 'text') {
    ...
    if (section.questions.length < MIN_CONSTRAINTS.quiz) {
      return { valid: false, error: `"${path}.questions" must contain at least ${MIN_CONSTRAINTS.quiz} questions (found ${section.questions.length})` };
    }
    ...
    if (section.sentences.length < MIN_CONSTRAINTS.fillBlank) {
      return { valid: false, error: `"${path}.sentences" must contain at least ${MIN_CONSTRAINTS.fillBlank} sentences (found ${section.sentences.length})` };
    }
    ...
    if (section.pairs.length < MIN_CONSTRAINTS.matching) {
      return { valid: false, error: `"${path}.pairs" must contain at least ${MIN_CONSTRAINTS.matching} matching pairs (found ${section.pairs.length})` };
    }
    ...
    if (section.items.length < MIN_CONSTRAINTS.sorting) {
      return { valid: false, error: `"${path}.items" must contain at least ${MIN_CONSTRAINTS.sorting} items to sort (found ${section.items.length})` };
    }
    for (let j = 0; j < section.items.length; j++) {
      const item = section.items[j];
      if (!item || typeof item !== 'object' || Array.isArray(item)) {
    ...
    if (section.blanks.length < MIN_CONSTRAINTS.cloze) {
      return { valid: false, error: `"${path}.blanks" must contain at least ${MIN_CONSTRAINTS.cloze} cloze blanks (found ${section.blanks.length})` };
    }
